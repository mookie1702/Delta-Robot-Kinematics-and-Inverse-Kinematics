...
void CHLMotionPlan::SetPlanPoints(PosStruct startPos, PosStruct endPos){
	...
	mStartMatrixData[11] = startPos.z;
	...
	SetRobotEndPos(startPos.x,startPos.y, startPos.z, startPos.yaw, startPos.pitch, startPos.roll);
	...
	SetRobotEndPos(endPos.x, endPos.y, endPos.z, endPos.yaw, endPos.pitch, endPos.roll);
	GetJointAngles(angle1, angle2, angle3, angle4);
...
void CHLMotionPlan::GetPlanPoints_line(){
	...
	ofstream outfile;		// 创建文件
	outfile.open("mydata_spatial.txt");
